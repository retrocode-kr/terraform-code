...
locals {
  create_irsa      = var.create_irsa && var.create
...
data "aws_iam_policy_document" "irsa" {
  ...
  statement {
    ...
    resources = [
      ...
      "arn:${local.partition}:ec2:*:${local.account_id}:insatnce/*",
  ...
  dynamic "statement" {
    for_each = local.enable_scheduler ? [1] : []
...
################################################################################
locals {
  enable_scheduler = var.enable_scheduler && var.create

  queue_name = coalesce(var.queue_name, "Karpenter-${var.cluster_name}")
}

resource "aws_sqs_queue" "this" {
  count = local.enable_scheduler ? 1 : 0
...
locals {
  events = {
    ...
    instance_state_change = {
      ...
      description = "Karpenter interrupt - EC2 instance state notification"
      event_pattern = {
        source      = ["aws.ec2"]
...
resource "aws_cloudwatch_event_rule" "this" {
  for_each = {for k, v in local.events : k => v if local.enable_scheduler}
...
locals {
  ...
  iam_role_name          = coalesce(var.iam_role_name, "${var.cluster_name}-karpenter")
