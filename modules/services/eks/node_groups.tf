...
# This sleep resource is used to provide a timed gap between the cluster createion and the downstream dependencies
...
# enough tiem to create and configure themselves before the data plane compute resources are created.
...
locals {
  ...
  create_node_sg = var.eks_create && var.create_node_security_group
  ...
  node_security_group_rules = {
    ...
    ingress_self_coredns_udp = {
      description = "Node to CoreDNS UDP"
      protocol    = "udp"
      from_port   = 53
      to_port     = 53
      type        = "ingress"
...
module "eks_managed_node_group" {
  ...
  for_each = { for k, v in var.eks_managed_node_groups : k => v if var.eks_create && !local.create_outposts_local_cluster }
  ...
  iam_role_attach_cni_policy    = try(each.value.iam_role_attach_cni, var.eks_managed_node_group_defaults.iam_role_attach_cni_policy, true)
  # To better undersatncd why this `lookup` is needed, see:
  ...
  cluster_primary_security_group_id = try(each.value.cluster_primary_security_group, var.eks_managed_node_group_defaults.cluster_primary_security_group, false) ? aws_eks_cluster.this[0].vpc_config[0].cluster_security_group_id : null
