...
resource "aws_ec2_tag" "cluster_primary_security_group" {
  ...
  for_each = { for k, v in var.cluster_tags :
...
locals {
  ...
  cluster_security_group_rules = { for k, v in {
    ingress_nodes_443 = {
      description              = "Allow nodes to communicate with each other for cluster API"
      from_port                = 443
      to_port                  = 443
      protocol                 = "tcp"
...
locals {
  ...
  # TODO - hopefully this can be once the AWS endpoint is named properly in China
...
data "aws_iam_policy_document" "assume_role_policy" {
  count = local.create && local.create_iam_role ? 1 : 0
...
resource "aws_iam_role" "this" {
  ...
  # Resources runnig on the cluster are still generating logs when destroying the module resources
  # which results in the log group being re-created even after Terraform destroy it. Removing the
  ...
  tags = merge(var.cluster_tags, var.iam_role_tags)
}
...
resource "aws_iam_role_policy_attachment" "cluster_encryption" {
  # Encryption config is not available in outposts
...
resource "aws_iam_policy" "cluster_encryption" {
  ...
  tags = merge(var.cluster_tags, var.cluster_encryption_policy_tags)
}
...
locals {
  node_iam_role_arns_non_windows = distinct(compact(concat(
...
resource "kubernetes_config_map_v1_data" "aws_auth" {
  count = local.create && var.manage_aws_auth_configmap ? 1 : 0
