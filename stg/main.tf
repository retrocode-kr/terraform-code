...
locals {
  ...
  password_data   = jsondecode(file("../stg/password.json"))
...
module "stg_vpc" {
  ...
  private_transit_gateways = [{
    cidr               = "172.12.10.0/23"
    transit_gateway_id = data.aws_ec2_transit_gateway.central_stg_tgw.id
    },
    {
...
resource "aws_ec2_transit_gateway_vpc_attachment" "example" {
  subnet_ids                                      = module.stg_vpc.private_subnet_ids
  transit_gateway_id                              = data.aws_ec2_transit_gateway.main.id
  vpc_id                                          = module.stg_vpc.vpc_id
  transit_gateway_default_route_table_association = false
  transit_gateway_default_route_table_propagation = false
  tags = {
    Name = "EV-SYSTEM-TGW-STG-ATTACHMENT"
  }
}

data "aws_ec2_transit_gateway" "central_stg_tgw" {
  filter {
    name   = "tag:Name"
    values = ["CENTRAL-STG-TGW"]
  }
}

resource "aws_ec2_transit_gateway_vpc_attachment" "central_stg_attachment" {
  subnet_ids         = module.stg_vpc.private_subnet_ids
  transit_gateway_id = data.aws_ec2_transit_gateway.central_stg_tgw.id
  vpc_id             = module.stg_vpc.vpc_id

  tags = {
    Name = "CENTRAL-STG-TGW-ATTACHMENT"
  }
}
...
data "aws_subnets" "bastion" {
  ...
  filter {
    name   = "vpc-id"
    values = [module.stg_vpc.vpc_id]
    # 테라폼 클라우드 outputs 사용시 
    #values = [data.tfe_outputs.stg.values.vpc_id]
...
data "aws_ami" "pri_bastion" {
  ...
  filter {
    name   = "name"
    values = ["EV-STG-BASTION-backup-2023-05-31-09-00-17"]
  }

}

module "stg_pri_bastion" {
  source                  = "../modules/services/instances"
  vpc_id                  = module.stg_vpc.vpc_id
...
# STG Aurora
###################################################################################################################
module "stg_database_aurora" {
  source                              = "../modules/services/database"
  availability_zones                  = var.avaliable_zones
  vpc_id                              = module.stg_vpc.vpc_id
  rds_type                            = var.rds_type
  env_name                            = var.env_name
  database_subnet_group_name          = module.stg_vpc.database_subnet_group_name[0]
  ...
  password                            = local.password_data.stg_aurora_master_password # password 는 json file
